-- =====================================================================
-- Migration: 20260906050000_calendario_filtra_conta_stone_recebiveis.sql
-- Etapa 4 da auditoria / universos do calendário
--
-- Problema
-- --------
-- A CTE `recebiveis` lia public.raw_stone_recebiveis sem nenhum filtro, enquanto
-- as vendas do mesmo calendário vêm de public.recebimento_stone_net, que aplica
-- COALESCE(sc.ativa AND sc.entra_faturamento, true) sobre public.stone_conta.
-- Duas contas Stone estão hoje com entra_faturamento = false. Nenhuma delas tem
-- recebíveis importados no momento, então o risco é latente: bastaria importar
-- um arquivo dessas contas para o calendário projetar recebíveis de uma conta
-- excluída do faturamento, comparando universos diferentes.
--
-- Correção
-- --------
-- Aplica na CTE `recebiveis` exatamente o mesmo filtro de conta usado pelas
-- vendas. Conferido em produção antes de escrever esta migration: com e sem o
-- filtro, os 442 dias e os totais de crédito e débito são idênticos, então a
-- mudança elimina o risco sem alterar nenhum número atual.
--
-- Não se aplica filtro por public.fonte_financeira aqui de propósito: a chave
-- 'stone_recebiveis' está com entra_faturamento e entra_caixa em false, e
-- condicionar a CTE a essas flags zeraria todos os recebíveis projetados.
-- Essa assimetria de configuração precisa ser resolvida antes, à parte.
--
-- Somente a CTE `recebiveis` muda; o restante da função é a definição
-- implantada, preservada carácter a carácter.
-- =====================================================================
...
begin
  ...
  with cortes as (
    ...
    left join public.stone_conta sc on sc.stonecode = r.stonecode
...
comment on function public.listar_calendario_financeiro(date) is
  'Calendário financeiro do mês. Recebíveis Stone respeitam o mesmo filtro de conta aplicado às vendas, evitando universos diferentes entre realizado e projetado.';

-- Validação: o filtro novo precisa estar presente e as garantias já conquistadas
-- pela migration 20260906000000 não podem ter sido perdidas na regravação.
do $validacao$
declare
  v_src text;
begin
  ...
  if position('public.stone_conta sc' in v_src) = 0 then
    raise exception 'Validacao falhou: filtro de conta Stone ausente nos recebiveis.';
  end if;
